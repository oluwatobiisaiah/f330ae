...
export default function PrefillPanelModal({
  ...
  const findFormDependencies = (currentNodeId: string, graphData: GraphResponse) => {
    const direct: Form[] = [];
    const transitive: Form[] = [];
    const visited = new Set<string>();

    const incomingEdges = graphData.edges.filter(edge => edge.target === currentNodeId);

    incomingEdges.forEach(edge => {
      const sourceNode = graphData.nodes.find(node => node.id === edge.source);
      if (sourceNode) {
        const sourceForm = graphData.forms.find(form => form.id === sourceNode.data.component_id);
        if (sourceForm) {
  ...
  const globalData = [
    { id: 'global.currentDate', label: 'Current Date', description: 'The current date' },
    { id: 'global.currentUser', label: 'Current User', description: 'The currently logged in user' },
    { id: 'global.journeyId', label: 'Journey ID', description: 'The ID of the current journey' },
    { id: 'global.organizationName', label: 'Organization Name', description: 'Name of the organization' },
    { id: 'global.organizationId', label: 'Organization ID', description: 'ID of the organization' },
  ];
  ...
  // Generate form field items from a form
                  ...
                  <List dense>
                    {filterItems(getFormFields(dependencyForm)).map((field) => (
                      <ListItem key={field.id} disablePadding>
                        <ListItemButton onClick={() => handleSelectMapping(field.id)}>
                          <ListItemText
                            primary={field.label}
