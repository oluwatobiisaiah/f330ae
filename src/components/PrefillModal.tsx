...
export default function PrefillModal({
    ...
    const handleFieldChange = (event: React.ChangeEvent<HTMLInputElement> | { target: { value: string } }) => {
    ...
    const isMappingReference = (value: string) => {
        return typeof value === 'string' && (
            value.startsWith('global.') ||
            (graphData?.forms.some(form => value.startsWith(form.id + '.')))
        );
    };
    const getFormattedMappingInfo = (value: string) => {
            ...
            <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers>
                    <Box sx={{ mb: 3 }}>
                        ...
                        <FormControl fullWidth sx={{ mb: 2 }} error={!!error} disabled={!!editingField}>
                    ...
                    {Object.keys(values).length === 0 ? (
                        ...
                        <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
                            {Object.entries(values).map(([field, value]) => {
                                const isMapped = isMappingReference(value.toString());
                                const { formName, fieldName } = isMapped
                                    ? getFormattedMappingInfo(value.toString())
                                    : { formName: '', fieldName: '' };
